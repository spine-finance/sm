...
contract BondMM is IBondMM, ERC20, Pausable {
    ...
    function quoteTokenAmount() public view returns (uint256) {
    ...
    function addLiquidity(
        ...
        quoteToken.transferFrom(lp, address(this), cashIn);
    ...
    function withdrawLiquidity(
        ...
        _burn(lp, _lpShares);
        //transfer out
        quoteToken.transfer(lp, cashOut);
    }
    ...
    function redeem(
        ...
        quoteToken.transfer(account, cashOut);
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        quoteToken.transfer(account, amountOut);
        quoteToken.transfer(vault, swapFee);
        _updateX(poolState.y, delta_y, 1, poolState.alpha);
        ...
        returns (uint256 amountIn)
    {
        ...
        quoteToken.transferFrom(account, address(this), uint_delta_y);
        quoteToken.transferFrom(account, vault, swapFee);
        bond.mint(account, maturity, _amountOut);
        _updateX(poolState.y, delta_y, 0, poolState.alpha);
        ...
        returns (uint256 amountOut)
    {
        ...
        quoteToken.transferFrom(account, address(this), _amountIn);
        quoteToken.transfer(vault, swapFee);
    ...
    function _updateX(
        ...
        X = y_new * ((y / y_new).pow(alpha) * (X / y + ud(ONE)) - ud(ONE));
    }
